...
**🚨 重要要求 - 题号处理（最关键）**：
- **🚨 绝对禁止**在JSON中保留任何主题目序号！子题内部序号需按规则智能处理！
- **🚨 必须删除**所有主题目前面的序号，如"1."、"2."、"8."等，以及大题序号"（1）"、"（2）"、"（3）"等
- **🚨 必须删除**解析和答案中的题号，如"31."、"32."、"33."等
- **🚨 必须删除**大题标号，如"一、"、"二、"、"三、"、"四、"、"五、"等
- **🚨🚨🚨 子题序号智能处理**：单一问题删除序号，多问题将"（1）"、"（2）"标准化为"1."、"2."格式
- **常见错误示例**：
...
- **🚨🚨🚨 子题序号智能处理说明**：
  ...
  * **识别标准**：只要在同一个子题content中出现"（1）"、"（2）"等连续序号，就必须应用此规则
- **🔥🔥🔥 重要区分示例**：
...
**🚨🚨🚨下划线转换规则（🔥🔥🔥极其重要，最高优先级，必须严格执行）**：

**⚠️ 核心原则：下划线转换必须基于题型判断结果**

**🔥 必须转换为 `<input>` 标签的情况**：
1. **type="填空"的题目**：无论是独立题目还是子题，所有 `______` 都必须转换
   - ✅ 正确：转换为 `<input size="XX" readonly="readonly" type="underline">`
   - 🚨 特别注意：不管下划线前后有什么符号（①②③、引号、括号等），都必须转换！

**🔥 绝对不转换的情况**：
1. **type="单选"/"多选"题**：所有 `______` 和 `（）` 保持原样（这些是选择题的答题标记）
   - 🚨 **重要**：单选题content中必须包含 `（）` 答题标记
2. **主题目摆设**：有subQuestions的主题目content中的 `______`（这些只是题目描述，不是真正的填空）
3. **type="简答"题中的下划线**：不转换，而是添加 `<full-line-blank>` 标签
...
**🚨 执行检查清单**：
解析每道题时必须按顺序执行：
1. [ ] 先识别题型（优先看accessories，再看内容特征）
2. [ ] 根据题型决定格式转换（单选保持下划线+括号，填空转换为input）
3. [ ] **单选/多选题特别检查**：确保content中包含答题括号 `（）`
4. [ ] 验证answer格式与type的匹配性  
5. [ ] 确保字段完整性（单选有accessories，填空无accessories）
6. [ ] 执行最终一致性检查
...
- **🚨 语句排序题特别注意**：
  ...
  * 这是最容易出错的地方，必须严格区分！
**🚨 单选题content和accessories分工规则（极其重要）**：

**普通单选题**：
- **content**：只包含题目要求 + 答题括号`（）`，不包含具体选项内容
...
4. **🔥 波浪线格式转换检查（第四优先级）**：
   ...
   - **🌊 波浪线强调**：`[文字内容]{.wavy-underline}` → `<u style=\"text-decoration-style: wavy;\">文字内容</u>`
...
- [ ] **避免子题序号格式错误**：单一问题删除序号，多问题子题将"（1）"、"（2）"改为"1."、"2."格式
