...
- **🚨🚨🚨 子题序号标准化示例（极其重要）**：
  * ❌ 错误：`<p>（1）请根据提示补充①②处的内容</p><p>（2）有人认为文中"母亲给父亲打下手"这段文字可以删去</p>`
  * ✅ 正确：`<p>1.请根据提示补充①②处的内容</p><p>2.有人认为文中"母亲给父亲打下手"这段文字可以删去</p>`
  * ❌ 错误：`<p>（1）卒中往往语，皆指目陈胜。</p><p>（2）良欲往从之，道遇沛公。</p>`
  * ✅ 正确：`<p>1.卒中往往语，皆指目陈胜。</p><p>2.良欲往从之，道遇沛公。</p>`
- **处理原则**：
  * **主题目序号**：删除所有数字序号（1. 2. 3. ... 19. 25. 等）和中文序号（（一）、（二）、（三）、一、二、三等）
  * **子题内序号**：如果子题content包含多个小问题，将"（1）"、"（2）"标准化为"1."、"2."
  * **题型名称**：保留题型名称但去掉序号（如"名著阅读"、"默写"等）
  * **分值信息**：可以保留或删除
- **说明**：
  * **主题目和大题**：无论什么题型，都必须删除题号，只保留题目内容本身
  * **子题内部**：如果包含多个小问题，需要将序号标准化为"1."、"2."格式而不是删除
  * **解析和答案**：其中的题号也必须删除
- **🚨🚨🚨 子题序号标准化说明**：
  * **适用场景**：当子题content中包含多个小问题时（如翻译题、填空题的多个空等）
  * **转换规则**：将括号序号"（1）"、"（2）"、"（3）"等统一改为点号序号"1."、"2."、"3."等
...
- **🔥🔥🔥 重要区分示例**：
  ...
  * **子题序号标准化**：`<p>（1）卒中往往语，皆指目陈胜。</p>` → `<p>1.卒中往往语，皆指目陈胜。</p>`
- **🚨🚨🚨 子题作答区域分配示例（极其重要）**：
  * ❌ **错误格式**（统一作答区域）：
    ```html
    <p>1.卒中往往语，皆指目陈胜。</p>
...
**✅ 正确转换示例（必须严格执行）：**
- ✅ **正确格式**：`<span style="border-bottom: 1px #000 solid; padding-left: 20px; padding-right: 20px">①</span><input size="15" readonly="readonly" type="underline">`
- ✅ **正确格式**：`①<input size="15" readonly="readonly" type="underline">`
- ✅ **正确格式**：`"<input size="20" readonly="readonly" type="underline">"`
- ✅ **正确格式**：`营造了 ②<input size="15" readonly="readonly" type="underline">的意境`
- ✅ **正确格式**：`<p>俯身看宁静碧绿的池水，<span style="border-bottom: 1px #000 solid; padding-left: 20px; padding-right: 20px">①</span><input size="20" readonly="readonly" type="underline">。</p>`
...
- [ ] **子题序号标准化检查**：确认子题content中的"（1）"、"（2）"已改为"1."、"2."格式
...
- [ ] **避免子题序号格式错误**：子题content中的"（1）"、"（2）"必须改为"1."、"2."格式，不能保留括号格式
