...
**⚠️ 题目数量统计严格规则**：
- 每个模块的总题数 = 该模块中最后一道题的 `questionNumber` 字段值
- 绝对禁止将子题（subQuestions）计算在内
- 统计时只计算主题目（question）的数量，不计算子题数量
- 如果发现统计结果异常（如听力部分超过10道题），必须重新检查
- **验证方法**：检查每个模块的题目数量是否合理，听力部分通常不超过10道题
...
**⚠️ 特别重要检查项**：
1. **答案格式一致性检查（🚨 最重要）**：
   - **⚠️ 必须与pandoc转换结果完全一致**：答案格式必须严格按照pandoc转换结果输出，绝对不能遗漏任何内容
   - **⚠️ 禁止修改答案格式**：不能随意添加、删除或修改答案中的任何字符、符号、括号内容
   - **🚨 括号备选答案保留要求（极其重要）**：
     * ✅ **必须完整保留**：答案中的备选答案必须完整保留，不能删除或忽略
     * ❌ **严重错误示例**：`目标定位（或教育目标）` → `目标定位` （删除了括号备选答案）

     * ✅ **正确保留示例**：`目标定位（或教育目标）` → `"目标定位（或教育目标）"`
   ...
   - **示例**：
     ...
     * pandoc：`【答案】①政策文件 ②目标定位（或教育目标） ③实践探索（或共同行动）` → JSON：`"blanks": ["政策文件", "目标定位（或教育目标）", "实践探索（或共同行动）"]`
...
9. **🚨 连接性说明文字完整性检查（新增重要项）**：
   - **🚨 极其重要**：检查所有题目是否保留了连接性说明文字
   - **常见被忽略的重要内容**：
     * "阅读上面的材料，完成下题" → 必须保留
     * "根据材料，完成下面小题" → 必须保留
     * "阅读下面的材料，回答问题" → 必须保留
     * "结合上述内容，完成以下题目" → 必须保留
   - **检查方法**：逐一核对每个题目，确保这类过渡语句没有被删除
   - **重要性说明**：这些语句虽然看似简单，但它们明确了题目与材料的关系，是题目完整性的重要组成部分

10. **括号提示保留要求（🚨 重要）**：
   - **🚨 必须保留所有备选答案**：答案中的括号备选表述（如"（或教育目标）"、"（或共同行动）"等）必须完整保留
   - **括号提示位置**：括号提示应放在对应的填空处附近，帮助学生理解应该填写什么
   - **示例格式**：
     * 填空提示：`<input type="underline" size="8" /> (create)` 或 `(create) <input type="underline" size="8" />`
     * 备选答案：`"目标定位（或教育目标）"` → 必须完整保留
   - **🚨 禁止删除任何括号内容**：绝对禁止删除任何括号内的提示信息或备选答案，这些都是题目的重要组成部分
...
**⚠️ 最终答案格式检查（🚨 绝对不能忽略）**：
- **🚨 答案必须与pandoc转换结果完全一致**：不能修改任何格式、符号或内容，特别是括号内的备选答案
- **🚨 括号备选答案检查（最重要）**：
  * ✅ **必须保留所有备选答案**：答案中出现的"（或XX）"格式必须完整保留
- **常见错误示例**：
  * ❌ `(s)ince` → `since` （删除了括号和首字母提示）
  * ❌ `inter_______` → `esting` （删除了部分提示内容）
  * ❌ `learn_______` → `ing` （删除了部分提示内容）
  * ❌ **删除"或"表述**：`政策文件、目标定位（或教育目标）、实践探索（或共同行动）` → 仅输出基础答案，忽略备选项
- **🚨 二选一作文题答案完整性**：
  * ✅ **必须保留所有题目选项**：二选一作文题的答案不能只有一个题目，要完整保留两个选择；答案也应该是两个例文而不是一个。
  * ❌ **错误做法**：将二选一作文拆分为subQuestions或拆分成两道独立的题
- **正确做法**：pandoc显示什么，JSON中就输出什么，完全一致，绝对不能遗漏备选答案信息
...
- **🚨 答案内容完整性（最关键）**：确保所有答案内容都完整，特别是括号内的备选答案信息，没有被截断或忽略
- **转义完整性**：确保所有特殊字符都正确转义
- **🚨 备选答案检查**：特别检查所有包含"（或XX）"、"(or XX)"等格式的答案是否完整保留

- **检查方法**：
  * 确保每个字符串都以 `"` 开始，以 `"` 结束，内容完整
  * 逐一核对每个答案的内容，确保备选答案信息没有丢失
  * 特别注意二选一题目的答案完整性
...
**⚠️ 题目说明提取规则（最重要）**：
- **🚨 题目开头说明必须完整**：不能遗漏任何题目说明文字，包括连接性说明
- **🚨 连接性说明文字必须保留（重要）**：如"阅读上面的材料，完成下题"、"根据材料，完成下面小题"、"阅读下面的材料，回答问题"等连接性表述必须完整保留
...
- **🚨 常见遗漏内容（特别注意）**：
  ...
  * 题目说明中的答题要求
  * 题目结尾的注意事项
  * **连接性说明文字**：如"阅读上面的材料，完成下题"等过渡语句
  * **材料引导语**：如"根据以下材料"、"结合上述内容"等
- **🚨 检查方法**：确保每个题目的content字段都包含完整的题目说明，特别检查是否保留了连接材料和题目的过渡语句
...
{
  "question": {
    "content": "<p>五、文段表达（10分）<br>从下面两个题目中任选一题，根据所给的中文和英文提示，完成一篇不少于50词的英语文段写作。文中已给出的内容不计入总词数。所给提示词语仅供选用。请不要写出真实的校名、姓名和地名。</p>",
    "type": "简答"
  },
  "subQuestions": [
    {
