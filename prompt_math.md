...
**学科和学段信息处理要求**：
- **必须为每个题目JSON添加四个参数**：`subjectId`、`phaseId`、`title` 和 `questionNumber`
...
- [ ] **层级结构检查**：JSON的顶层应该包含`subjectId`、`phaseId`、`title`、`questionNumber`、`question`、`subQuestions`（如果有）
...
### ✅ 结构完整性验证
- [ ] **每个题目包含完整字段**：必须包含subjectId、phaseId、title、questionNumber四个字段，根据试卷内容分析确定学科、学段、模块名称和序号
