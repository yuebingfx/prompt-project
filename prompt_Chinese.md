...
试卷中题组名、题目和子题的关系阐述如下：
1.题组名和题目是1对n的关系：一个题组名可能对应一道题目，也可能对应多道题目，如果对应一道题目，则题组名通常被放在对应的题目的content中去，如果对应的是多道题目，且每个题目之间相对独立（例如题组名是：三、现代文阅读，题组名下包含若干题目），则每道题目都需要被拆分成一道题（每道题可能包含若干子题），此时题组名不需要放在题目的content中去，除非题组名对做题有影响。
...
- **模块名称信息（module）**：根据试卷中的题组名称确定
  * 如"一、基础运用"、"二、古诗文阅读"、"三、名著阅读"、"四、现代文阅读"、"五、作文"等
...
- **🚨🚨🚨 下划线转换**：默写题子题中的`______`必须转换为`<input>`标签

**🚨 判断题拆分规则（极其重要）**：

**情况：某个子题包含多个判断小题**
- **🔥 关键处理**：将这一个包含多个判断的子题 → 拆分为多个平级的判断子题
- **拆分原则**：
  * 删除原来包含多个判断的子题
  * 在同一位置插入多个独立的判断子题
  * 每个新子题type为"判断"
...
- **❌ 错误做法**：将多个小题放在一个子题中，answer使用"1,0,1"格式
...
{
  "question": {
    "content": "<p>请认真听录音朗读短文两遍，然后按要求回答下列问题。</p>",
    "type": "综合"
  },
  "subQuestions": [
    {
      "content": "<p>汉服，是汉民族的传统服装，又称<input size=\"20\" readonly=\"readonly\" type=\"underline\">。</p>",
      "type": "填空",
      "answer": {"blanks": ["略"]}
    },
    {
      "content": "<p>根据录音内容判断对错，对的打"√"，错的打"×"。</p><p>1.唐代服饰用色浓烈大方。</p><p>2.宋代汉服完全一样。</p><p>3.汉服回归现代生活。</p>",
      "type": "判断",
      "answer": {"choice": "1,0,1"}  // ❌ 错误！
    }
  ]
}
```
...
```json
{
  ...
  "subQuestions": [
    ...
    {
      "content": "<p>根据录音内容判断对错，对的打“√”，错的打“×”。唐代服饰用色浓烈大方，呈现出雍容华美的特点。</p>",
      "type": "判断",
      "answer": {"choice": "1"},
      "solution": "<p>本题考查对录音内容的理解和判断能力。</p>"
    },
    {
      "content": "<p>宋代汉服在形制设计和风格上同唐代汉服完全一样。</p>",
      "type": "判断",
      "answer": {"choice": "0"},
      "solution": "<p>本题考查对录音内容的理解和判断能力。</p>"
    },
    {
      "content": "<p>有人认为，让汉服回归现代生活才是对其最好的保护与传承。</p>",
      "type": "判断",
      "answer": {"choice": "1"},
      "solution": "<p>本题考查对录音内容的理解和判断能力。</p>"
    }
...
**🔥 关键要点**：
- 原来的1个包含3个判断的子题 → 拆分为3个独立的判断子题
- 每个判断子题都是平级的subQuestion
...
- **🚨🚨🚨 必须避免的严重错误示例**：
  * ❌ **主题目序号错误**：`<p>19. 班级准备开展\"有趣的《西游记》\"读书分享会</p>`
  * ✅ **正确**：`<p>班级准备开展\"有趣的《西游记》\"读书分享会</p>`
  * ❌ **子题序号错误**：`<p>（1）峰峰的发言中有几处错误，请你用修改符号改正。</p>`
  * ✅ **正确**：`<p>峰峰的发言中有几处错误，请你用修改符号改正。</p>`
  * ❌ **子题序号错误**：`<p>（2）同学们对你的名字很感兴趣，请你也来讲讲...</p>`
  * ✅ **正确**：`<p>同学们对你的名字很感兴趣，请你也来讲讲...</p>`
  * ❌ **大题序号错误**：`<p>四、名著阅读（6分）</p>`
  * ✅ **正确**：`<p><strong>名著阅读</strong></p>`
- **🚨🚨🚨 子题序号智能处理示例（极其重要）**：
  * **🔸 单一问题子题示例**（如默写题）：
    - ❌ 错误：`<p>1.______，非志无以成学。</p>`
    - ✅ 正确：`<p><input size="30" readonly="readonly" type="underline">，非志无以成学。</p>`
  * **🔸 多问题子题示例**（如翻译题）：
    - ❌ 错误：`<p>（1）卒中往往语，皆指目陈胜。</p><p>（2）良欲往从之，道遇沛公。</p>`（括号格式错误）
    - ✅ 正确：`<p>1.卒中往往语，皆指目陈胜。</p><p>2.良欲往从之，道遇沛公。</p>`（标准化序号）
- **🚨 处理原则**：
  * **主题目序号**：删除所有数字序号（1. 2. 3. ... 19. 25. 等）和中文序号（（一）、（二）、（三）、一、二、三等）
  * **🚨 子题序号**：根据子题内容智能处理，单一问题删除序号，多问题标准化序号
  * **🚨🚨🚨 阅读材料内容序号保护**：阅读材料中的①②③④⑤⑥⑦⑧等段落序号必须保留
  * **题型名称**：保留题型名称但去掉序号（如"名著阅读"、"默写"等）
  * **分值信息**：可以保留或删除
- **说明**：
  * **主题目和大题**：无论什么题型，都必须删除题号，只保留题目内容本身
  * **🚨 子题序号智能处理**：单一问题删除序号，多问题标准化序号格式
  * **🚨 阅读材料保护**：小说、文言文、诗词等原文内容中的段落编号必须保留
  * **解析和答案**：其中的题号也必须删除
- **🚨🚨🚨 子题序号智能处理说明**：
  * **单一问题子题**：如果子题只包含一个问题，删除所有序号（如"1."、"2."等）
  * **多问题子题**：如果子题包含多个小问题，将"（1）"、"（2）"、"（3）"标准化为"1."、"2."、"3."
  * **🔥🔥🔥 作答区域分配规则**：每个小问题后面必须紧跟独立的作答区域，不能统一放在最后
  * **注意事项**：这个规则只适用于子题内部，主题目的题号依然需要完全删除
  * **识别标准**：只要在同一个子题content中出现连续序号的小题，就必须应用此规则
- **🔥🔥🔥 重要区分示例**：
  * **主题目序号删除**：`<p>8. 这篇文章的主旨是什么？</p>` → `<p>这篇文章的主旨是什么？</p>`
  * **🚨 子题序号删除**：`<p>（1）卒中往往语，皆指目陈胜。</p>` → `<p>卒中往往语，皆指目陈胜。</p>`
- **🚨🚨🚨 子题作答区域分配示例（极其重要）**：
  * ❌ **错误格式**（统一作答区域）：
    ```html
    <p>卒中往往语，皆指目陈胜。</p>
    <p>良欲往从之，道遇沛公。</p>
    <p style="overflow: hidden;"><full-line-blank...></p>
    ```
  * ✅ **正确格式**（每句后跟作答区域）：
    ```html
    <p>卒中往往语，皆指目陈胜。</p>
...
**🚨 重要要求 - 格式标记转换（极其关键）**：
- **🚨 重要区分**：只有特定的格式标记需要转换，不是所有【】内容！
- **🚨 必须转换的格式标记**：仅限【居中】【居右】【首行缩进】等文档排版标记
- **🚨 重要区分两种情况**：
  * **必须保留**：题目内容中的【A】【B】【C】【D】等占位符标记（如"打破时间【A】空间的限制"）
  * **需要删除**：选项前的A、B、C、D序号标记（accessories中只要选项内容，不要序号）
  * **必须保留**：【答案】【解析】【材料】等说明性标记
- **核心转换规则**：
...
- **❌ 严重错误示例**：
  ...
  * ❌ 严重错误：删除题目内容中的【A】【B】【C】【D】等占位符标记
  * ❌ 严重错误：在accessories中保留"A. 选项内容"格式
- **✅ 正确转换示例**：
  * ✅ 正确：`<p style="text-align: center;"><strong>小院的馈赠</strong></p>`
  * ✅ 正确：`<p>&nbsp; &nbsp;1937年4月，新华书店诞生于延安...</p>`
  * ✅ 正确：`<p style="text-align: right;">（取材于朱熹《与长子受之》）</p>`
  * ✅ 正确：保留`打破时间【A】空间的限制`中的【A】占位符标记
  * ✅ 正确：accessories中使用`<p>选项内容</p>`，不含A、B、C、D序号
- **🔥 检查要求**：输出前必须检查，确保只转换了排版格式标记，保留了题目内容标记！
...
这里储存了所有可能需要的排版指导，你有任何问题都可以求助于这里。
{
  ...
  "properties" : {
    "subjectId" : {
      "type" : "integer",
      "title" : "学科ID",
      "description" : "学科类型标识：语文1、数学2、英语3、物理4、化学5、生物6、政治7、历史8、地理9、音乐11、体育12、美术13、信息技术14、综合实践15"
    },
    "phaseId" : {
      "type" : "integer", 
      "title" : "学段ID",
      "description" : "学段类型标识：小学1、初中2、高中3、学前4、高等教育5"
    },
    "module" : {
      "type" : "string",
      "title" : "模块名称",
      "description" : "题组名称，如'一、基础运用'、'二、现代文阅读'等"
    },
    "questionNumber" : {
      "type" : "string",
      "title" : "题目编号",
      "description" : "该题目在对应模块中的序号，如'1'、'2'、'3'等"
    },
    "question" : {
...
{
  "question": {
    "content": "<p>下列句子中，标点符号使用有误的一项是（）</p><p>A．小裁缝回答说：\"好吧，我一定会制伏两个巨人的。\"</p><p>B．\"我认为我必须重新到外面的世界去\"：丑小鸭说。</p><p>C．\"我好几天没吃东西了，\"拇指姑娘对田鼠说道，\"请给我点吃的好吗？\"</p>",
    "type": "单选",
    "accessories": [
      "<p>小裁缝回答说：\"好吧，我一定会制伏两个巨人的。\"</p>",
      "<p>\"我认为我必须重新到外面的世界去\"：丑小鸭说。</p>",
      "<p>\"我好几天没吃东西了，\"拇指姑娘对田鼠说道，\"请给我点吃的好吗？\"</p>"
    ],  
    "answer": {
        "choice": "1"
      },
    "solution": "<p>这道题考查学生对标点符号使用的理解和掌握。</p>"
  }  
}// ❌❌❌ content和accessories重复了！这是严重错误！
```
...
{
  "question": {
    "content": "<p>下列句子中，标点符号使用有误的一项是（）</p>",
    "type": "单选",
    "accessories": [
      "<p>小裁缝回答说：\"好吧，我一定会制伏两个巨人的。\"</p>",
      "<p>\"我认为我必须重新到外面的世界去\"：丑小鸭说。</p>",
...
}// ✅ 正确：content只有题目要求，accessories包含选项内容
```
...
## 综合实践题目示例（包含多个子题）
{
  ...
  "subQuestions": [
    ...
    {
      "content": "<p><p><strong>【表演提示】</strong></p><p>经过排练，课本剧《愚公移山》即将上演。请你在舞台表现方面给演员一条提示。</p><p style=\"overflow: hidden;\"><full-line-blank id=\"mce_1\" style=\"display: inline; position: static;\" contenteditable=\"false\" data-lines=\"1\" data-punctuation=\"\" data-first-line-width=\"651\"></full-line-blank></p>",
      "type": "简答",
...
```json
{
  "content": "<p>汉服一脉相承，按时期可依次分为<input size=\"15\" readonly=\"readonly\" type=\"underline\">等。</p>",
  "type": "单选",
  "accessories": ["<p>唐制、魏晋风、宋制、明制汉服</p>", "<p>魏晋风、唐制、宋制、明制汉服</p>"],
  "answer": {"choice": "1"}
}
```
...
```json
{
  "content": "<p>汉服一脉相承，按时期可依次分为______等。（）</p>",
  "type": "单选",
  "accessories": ["<p>唐制、魏晋风、宋制、明制汉服</p>", "<p>魏晋风、唐制、宋制、明制汉服</p>"],
  "answer": {"choice": "1"}
}
...
❌ **错误示例2**（content和accessories重复选项内容）：
```json
{
  "content": "<p>下列句子中，标点符号使用有误的一项是（）</p><p>A.选项内容1</p><p>B.选项内容2</p>",
  "type": "单选",
  "accessories": ["<p>选项内容1</p>", "<p>选项内容2</p>"],  // ❌ 重复了！
  "answer": {"choice": "1"}
}
```

✅ **正确示例2**（content和accessories正确分离）：
```json
{
  "content": "<p>下列句子中，标点符号使用有误的一项是（）</p>",
  "type": "单选", 
  "accessories": ["<p>选项内容1</p>", "<p>选项内容2</p>"],
  "answer": {"choice": "1"}
}
```
...
```json
{
  "content": "<p>下列句子中，标点符号使用有误的一项是</p>",
  "type": "单选",
  "accessories": ["<p>选项A内容</p>", "<p>选项B内容</p>"],
  "answer": {"choice": "1"}
}
```
...
```json
{
  "content": "<p>根据拼音写汉字：自rán______</p>",
  "type": "填空",
  "answer": {"blanks": ["然"]}
}
```
...
```json
{
  "content": "<p>根据拼音写汉字：自rán<input size=\"8\" readonly=\"readonly\" type=\"underline\"></p>",
  "type": "填空",
  "answer": {"blanks": ["然"]}
}
```
...
**🔥🔥🔥 重要区分（极其关键）**：此拆分思路仅适用于**多个独立资料**的情况，与**单一共享材料**的阅读理解题完全不同！

**🚨🚨🚨 强制合并规则（最高优先级）**：
- **如果多道题目基于完全相同的材料内容**（如都基于"资料三"），**必须合并为一道题目 + subQuestions结构**
- **绝对禁止**将基于相同资料的题目拆分为独立题目
- **判断标准**：题目引用相同的资料编号（如"资料三"、"材料二"等）
- **关键识别**：题目开头或题干中出现"资料三"等相同标识

**适用场景**：当"基础·运用"板块包含**多个不同资料**且有共同背景时
- ✅ **多资料场景**：资料一、资料二、资料三...每个资料独立，且每个资料对应不同题目
- ❌ **单一材料场景**：只有一份阅读材料，多个题目基于同一材料 ← 这种情况必须合并为subQuestions！
- ❌ **相同资料多题场景**：多道题目都基于"资料三" ← 必须合并！
...
**❌ 错误拆分（绝对禁止）**：
```
题目A: 基于"资料三"的词义理解题（独立题目）
题目B: 基于"资料三"的成语使用题（独立题目）
```
...
{
  "question": {
    "content": "<p style=\"text-align: center;\">资料三</p><p>&nbsp; &nbsp;在北京市政府的支持下，有些书店迁入古建筑所在的院落。时尚的阅读空间与古树、古建筑相映成趣。这些书店承担着\"阅读传承\"与\"文物活化\"的双重使命，除图书外，还收藏老照片、旧报刊、胡同门牌等多种历史资料。图书与历史资料相得益彰，共同讲述着北京作为古城、古都的发展历史。这些书店大多自出心裁，选择具有地域特色的主题陈列图书和其他展品，如\"京味文学\"\"口述历史\"\"史地民俗\"等。读者捕风捉影，可以领略老北京的城市风貌。</p>",
    "type": "单选"
  },
  "subQuestions": [
    {
      "content": "<p>你想知道文段中\"活化\"的意思。查《现代汉语词典》，\"活化\"的解释为\"使分子或原子的能量增强\"。根据词典解释，下列对文段中\"活化\"的理解不正确的一项是（）</p>",
      "type": "单选",
      "accessories": ["<p>维持原有面貌</p>", "<p>开掘历史价值</p>", "<p>激发生机活力</p>", "<p>拓展现实意义</p>"],
      "answer": {"choice": "0"},
      "solution": "<p>本题考查词义辨析。A.文段中\"文物活化\"是让文物通过书店等形式发挥新价值，\"维持原有面貌\"未体现\"活化\"（使能量增强、赋予新活力等）；</p>"
    },
    {
      "content": "<p>你检查了文段中成语的使用情况。下列成语使用不恰当的一项是（）</p>",
      "type": "单选", 
      "accessories": ["<p>相映成趣</p>", "<p>相得益彰</p>", "<p>自出心裁</p>", "<p>捕风捉影</p>"],
      "answer": {"choice": "3"},
      "solution": "<p>本题考查成语运用。D.捕风捉影：比喻说话或做事时用似是而非的迹象做根据，含贬义。根据文段\"读者捕风捉影，可以领略老北京的城市风貌\"可知，想表达读者通过书店陈列感受北京风貌，是积极正向的，而\"捕风捉影\"带贬义，与语境情感、表意冲突，使用不恰当；</p>"
    }
  ]
}
```
...
**🔥🔥🔥 合并要点**：
1. **主题目包含完整的共享材料**：资料三的完整内容
2. **子题只包含具体问题**：不重复材料内容  
3. **避免材料重复**：材料只在主题目出现一次
4. **保持答案完整**：每个子题都有完整的答案和解析
...
**🔥🔥🔥 普通单选题（90%的情况）🔥🔥🔥**：
- **🔥 content**：只包含题目要求 + 答题括号`（）`，**绝对不包含任何选项内容**
- **🔥 accessories**：包含所有选项内容，无A、B、C、D序号
- **🚨 严重错误**：content和accessories内容重复是绝对禁止的！
- **🚨 检查要求**：content中不能出现A、B、C、D选项内容，只能有题目要求
- **🚨 重要澄清**：单选题的"题干"分为两部分：题目要求（放在content）+ 选项内容（放在accessories）

**🔥🔥🔥 语句排序题（特殊单选题，10%的情况）🔥🔥🔥**：
- **🔥 识别特征**：题目要求对句子进行排序，选项是①②③、①③②等排列组合
- **🔥 content**：包含题目要求 + 待排序的序号句子内容 + 答题括号`（）`
- **🔥 accessories**：只包含排列组合选项（如`①②③`、`①③②`等）
- **🚨 关键区分**：只有明确要求"排序"的题目才使用这种格式，其他都是普通单选题！

**🚨🚨🚨 重要提醒**：
- **90%的单选题都是普通单选题**，content绝对不能包含选项内容
- **只有语句排序题**才在content中包含句子内容
- **判断标准**：看选项是否是①②③排列组合，如果不是就是普通单选题
...
8. **语文学科特殊处理要求**：
   ...
   - **⚡⚡⚡ 复杂嵌套格式转换示例（必须严格遵循）⚡⚡⚡**：
     ...
     * **🚨正确示例：** `[【首行缩进】[文本]{.single-underline}]{.underline}` → `<p>&nbsp; &nbsp;<u>文本</u></p>` ✅ **这是正确的！**
   ...
   - **🔥 避免分析题作答空缺失**：
     * **分析题、解释题、说明题、阐述题等简答题必须包含作答空**
     * **作文题不需要作答空**
     * **判断依据**：通过题干和答案类型推断该题是否为作文题
     * **作文题特征**：一般位于卷纸的最后一道题，题目通常要求写作文、写议论文、写记叙文等
     * **简答题特征**：要求分析、解释、说明、阐述、谈谈看法等，这些题目都需要提供作答空间
...
🔥🔥🔥 **单选题结构检查（超最高优先级）** 🔥🔥🔥：
   - **🔥🔥🔥 首先判断题型**：90%是普通单选题，10%是语句排序题
   - **🔥🔥🔥 普通单选题检查**：content字段绝对不能包含A、B、C、D选项内容
   - **🔥🔥🔥 检查内容重复**：content和accessories绝对不能有重复内容
   - **🔥🔥🔥 正确格式**：普通单选题content只包含"题目要求（）"，accessories包含选项内容
...
🔥🔥 **加点字格式检查（超最高优先级）** 🔥🔥：
   ...
   - **🔥 严重错误检查**：DOT_BELOW加点字绝对不能被错误转换为波浪线格式`<u style="text-decoration-style: wavy;">内容</u>`（但{.wavy-underline}标记应该正确转换为波浪线）
0. **🔥 JSON转义检查（第零优先级）**：
...
3. **🚨 格式标记转换检查（第三优先级）**：
- **🔍 精确检查**：只转换排版格式标记【居中】【居右】【首行缩进】，保留题目内容标记
- **❌ 常见错误**：`<p><strong>【居中】标题</strong></p>` ← 这是严重错误
- **✅ 正确格式**：`<p style="text-align: center;"><strong>标题</strong></p>`
- **✅ 正确保留**：`打破时间【A】空间的限制`中的【A】占位符标记必须保留
- **✅ 正确删除**：accessories中的"A. 选项内容" → `<p>选项内容</p>`
- **强制要求**：只转换排版格式标记，保留题目内容占位符，删除选项序号
4. **🔥🔥 波浪线格式转换检查（第四优先级）🔥🔥**：
   - **🌊 波浪线填空**：`[&nbsp;&nbsp;&nbsp;...]{.wavy-underline}` → `<span style="text-decoration: underline wavy; text-underline-position: under;" data-mce-style="text-decoration: underline wavy; text-underline-position: under;">&nbsp;&nbsp;&nbsp;...</span>`
   - **🌊🔥 波浪线强调（必须转换）**：`[文字内容]{.wavy-underline}` → `<u style=\"text-decoration-style: wavy;\">文字内容</u>` **← 这个转换是必须的！**
   - **⚠️ 关键判断**：内容是纯&nbsp;=填空用span标签，内容是文字=强调用u标签
   - **🔥🔥 严重错误检查**：绝对不能忽略`{.wavy-underline}`标记！必须转换为波浪线格式！
   - **🔥 重要**：检查所有`[内容]{.single-underline}`是否都转换为`<u>内容</u>`
   - **🔥 重要**：绝对不能在最终JSON中保留任何`{.wavy-underline}`或`{.single-underline}`格式标记
   - **🔥 注意区分**：`{.wavy-underline}`是波浪线（必须转换），`[\[DOT_BELOW\]]`是加点字，完全不同！
   - **🔥 实际案例要求**：`[红头看见自己的朋友，高兴得流下了眼泪："谢谢你......"]{.wavy-underline}` → `<u style=\"text-decoration-style: wavy;\">红头看见自己的朋友，高兴得流下了眼泪："谢谢你......"</u>`
   - **🌊 波浪线填空错误示例**：
     * ❌ **错误**：`然而，<u style=\"text-decoration-style: wavy;\">。</u>在某种意义上` ← 将填空错误转换为强调
     * ✅ **正确**：`然而，<span style=\"text-decoration: underline wavy; text-underline-position: under;\" data-mce-style=\"text-decoration: underline wavy; text-underline-position: under;\">&nbsp;&nbsp;&nbsp;...</span>在某种意义上`
   - **实际错误示例**：
     * ❌ `<p>截zhì（ ）目前，我国全球重要...</p>`
     * ✅ `<p>截zhì<input type=\"bracket\" size=\"8\" />目前，我国全球重要...</p>`
     * ❌ `<p>古梨树历史yōu（ ）久</p>`
     * ✅ `<p>古梨树历史yōu<input type=\"bracket\" size=\"8\" />久</p>`
1. **🔥🔥🔥 题号序号删除检查（第一优先级，🚨 最关键）**：
   - **🔥 绝对禁止题目序号**：检查所有题目内容绝不能包含题号（如"1."、"2."、"8."、"19."、"25."等）
   - **🔥 绝对禁止子题序号**：检查所有子题绝不能包含序号（如"（1）"、"（2）"、"（3）"等）
   - **🔥 绝对禁止大题序号**：检查所有内容绝不能包含大题标号（如"一、"、"二、"、"三、"、"四、"、"五、"等）
   - **🔥 绝对禁止解析序号**：检查所有解析和答案绝不能包含题号（如"31."、"32."、"33."等）
   - **🚨🚨🚨 重要例外：阅读材料内容保护**：必须保留阅读材料中的段落序号（①②③④⑤⑥⑦⑧），这是原文内容的组成部分
     * ❌ **单一问题错误**：`<p>1.<input size="25" readonly="readonly" type="underline">...`
     * ✅ **单一问题正确**：`<p><input size="25" readonly="readonly" type="underline">...`
     * ❌ **多问题错误**：`<p>（1）翻译句子</p><p>（2）翻译句子</p>`
     * ✅ **多问题正确**：`<p>1.翻译句子</p><p>2.翻译句子</p>`
   - **实际错误示例**：
...
**🚨🚨🚨 序号删除强制要求（必须第一个检查）**：
**检查所有题目和子题是否还包含任何序号（如"1."、"2."、"（1）"、"（2）"等），如有发现必须删除！**

**在提交解析结果前，必须逐一检查以下项目**：

### 🔥🔥🔥 选择题验证（超重要）🔥🔥🔥
- [ ] **🔥🔥🔥 题型判断正确**：首先判断是普通单选题（90%）还是语句排序题（10%）
- [ ] **🔥🔥🔥 普通单选题content正确**：content只包含题目要求+答题括号，绝对不包含A、B、C、D选项内容
- [ ] **🔥🔥🔥 语句排序题content正确**：只有选项是①②③排列组合的题目才在content中包含句子内容
- [ ] **🔥🔥🔥 避免内容重复**：content和accessories绝对不能有重复内容
...
- [ ] **逐句作答区域检查**：如果简答题包含多个小问题（如翻译题），每个小问题后必须紧跟独立作答区域
...
- [ ] **subQuestions位置正确**：确认`subQuestions`与`question`并列，不在`question`内部
- [ ] **层级结构检查**：JSON的顶层应该包含`subjectId`、`phaseId`、`module`、`questionNumber`、`question`、`subQuestions`（如果有）
- [ ] **绝对禁止嵌套**：`question`对象内部绝对不能包含`subQuestions`字段
- [ ] **结构完整性**：每个题目都必须有正确的层级结构

### 🔥🔥🔥 题目拆分结构最终检查（第一优先级，极其重要）
- [ ] **相同资料强制合并检查**：检查是否有多道题目基于相同资料（如都基于"资料三"），必须合并为subQuestions
- [ ] **资料编号识别**：检查题目开头是否出现相同的资料标识（资料一、资料二、材料三等）
- [ ] **错误独立拆分检查**：绝对禁止将基于相同资料的题目拆分为独立题目
- [ ] **类型区分**：明确区分阅读理解类（单一材料）vs 基础运用类（多资料）
- [ ] **阅读理解类识别**：检查是否有"文中横线处"、"文中方框内"、"根据上面材料"等表述
- [ ] **错误拆分检查**：确认阅读理解类题目已合并为subQuestions，没有被误当作基础运用类拆分
- [ ] **材料依赖检查**：所有依赖前面阅读材料的题目都必须作为子题处理
- [ ] **完整性验证**：主题目包含完整阅读材料，子题包含具体问题
- [ ] **基础运用类检查**：如果是多个独立资料，只有共享同一资料的题目才合并

### 🚨 格式标记转换最终检查（第二优先级）
- [ ] **【居中】标记清除**：确认所有【居中】标记已转换为`style="text-align: center;"`
- [ ] **【居右】标记清除**：确认所有【居右】标记已转换为`style="text-align: right;"`
- [ ] **【首行缩进】标记清除**：确认所有【首行缩进】标记已转换为`<p>&nbsp; &nbsp;内容</p>`
- [ ] **子题序号智能处理检查**：单一问题删除序号，多问题将"（1）"、"（2）"改为"1."、"2."格式
- [ ] **逐句作答区域布局检查**：确认多个小问题的简答题，每个小问题后都有独立的作答区域
- [ ] **格式标记验证**：确保JSON中没有任何【】标记残留

### ✅ 结构完整性验证
- [ ] **每个题目包含完整字段**：必须包含subjectId、phaseId、module、questionNumber四个字段，根据试卷内容分析确定学科、学段、模块名称和序号
- [ ] **题目类型正确**：type字段准确反映题目性质（单选、填空、简答等）
- [ ] **答案格式匹配**：choice用于选择题，blanks用于填空题，answer用于简答题

### ✅ 内容准确性验证  
- [ ] **题目内容完整**：content字段包含完整的题干和说明（注意：单选题的"题干"分为题目要求+选项，选项放在accessories中）
...
```json
{
  ...
  "solution": "<p>本题考查文章内容概括。</p><p>根据\"得遇良师引领，明确研究方向\"可知对应文章第④至⑥段，吴良镛因《释\"阙\"》一文获得了梁思成、林徽因两位先生的认可，从而明确了自己的研究方向。第一个空应填入\"树立人生理想\"；第三个空应填入\"投身祖国建设\"。</p>"
}
```
...
**生成JSON时必须确保**：
- [ ] **JSON结构完整**：所有括号、方括号、引号都有配对
- [ ] **字段完整性**：所有必需字段都已生成完毕  
- [ ] **HTML标签配对**：solution中的所有HTML标签都有结束标签
- [ ] **字符正确转义**：特殊字符（如引号）已正确转义
- [ ] **无异常字段**：不存在单独的"阙"字段或其他无意义字段
- [ ] **内容完整性**：solution字段包含完整的解析，不被截断

### ❌ 常见错误防范
- [ ] **🔥🔥🔥 避免保留任何题目序号（最严重错误）**：绝对禁止在content中保留"1."、"2."、"（1）"、"（2）"等任何序号
- [ ] **🚨🚨🚨 重要例外：保护阅读材料内容序号**：必须保留小说、文言文、诗词等阅读材料中的段落序号（①②③④⑤⑥⑦⑧），这些是原文内容的组成部分
- [ ] **🔥🔥🔥 避免相同资料题目错误拆分（最严重错误）**：绝对禁止将基于相同资料（如都基于"资料三"）的题目拆分为独立题目，必须合并为subQuestions
- [ ] **🔥🔥🔥 避免资料编号混淆**：仔细检查题目开头的资料标识，相同标识的题目必须合并
- [ ] **🔥🔥🔥 避免【A】【B】【C】【D】处理混淆**：题目内容中的占位符必须保留，选项序号必须删除
- [ ] **🔥 避免类型混淆**：不要把阅读理解类（单一材料）误理解为基础运用类（多资料）而错误拆分
- [ ] **🔥 避免共享材料拆分**：基于同一阅读材料的多个题目必须合并，不能分别处理
- [ ] **避免格式错误**：选项必须用HTML标签包装，不能是纯文本

- [ ] **避免序号混淆**：choice答案索引从0开始，不是从1开始
- [ ] **🔥 避免简答题作答空缺失（最常见错误）**：简答题content中必须包含`<full-line-blank>`标签，如果原文没有下划线就在题目最后强制增加一个标签：`<p style=\"overflow: hidden;\"><full-line-blank id=\"mce_1\" style=\"display: inline; position: static;\" contenteditable=\"false\" data-lines=\"3\" data-punctuation=\"\" data-first-line-width=\"651\"></full-line-blank></p>`
- [ ] **避免逐句作答区域错误**：翻译题等包含多个小问题的简答题，不能将作答区域统一放在最后，必须每个小问题后紧跟独立作答区域
- [ ] **🔥 避免书写题作答空缺失**：书写题、临摹题、抄写题等简答题必须包含作答空，但是作文题不需要作答空
- [ ] **🔥 避免分析题作答空缺失**：分析题、解释题、说明题、阐述题等简答题必须包含作答空，但是作文题不足要作答空
- [ ] **🔥 避免简答题格式错误**：作答空标签必须完整，包含必要的属性
- [ ] **🚨🚨🚨 避免填空题填空框缺失（最严重错误）**：填空题content中必须包含`<input>`标签，绝对不能保留`______`格式
...
### 数据完整性最终检查
在输出JSON之前，必须进行以下最终检查：

1. **结构完整性**：确认所有JSON对象和数组都有正确的开闭标记
2. **字段完整性**：确认每个题目都包含所有必需字段（subjectId, phaseId, module, questionNumber等）
3. **内容完整性**：确认solution字段内容完整，不被截断，以完整的HTML标签结束
4. **字符正确性**：确认所有特殊字符已正确转义，引号配对正确
5. **无异常字段**：确认不存在"阙"等异常字段名
6. **语法正确性**：整个JSON可以被正确解析，没有语法错误
