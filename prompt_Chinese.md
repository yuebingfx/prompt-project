...
- **下划线填空（重要）**：
  ...
  * **type为"填空"的题目**：下划线必须转换为 `<input size="X" readonly="readonly" type="underline">` 标签
...
- **🚨 填空格式要求（最重要）**：
  ...
  * ✅ type为"填空"的题目：`<input size="X" readonly="readonly" type="underline">`
...
- **正确格式示例**：
  ...
  * type为"填空"的题目：`<input size="8" readonly="readonly" type="underline">`
  * 选择题括号填空：`（）`（保持原始格式）
...
**填空形式选择规则（重要）**：
- **简化原则**：根据type和结构决定填空处理方式
- **具体规则**：
  * **包含子题的主题目**：保留原始`______`下划线格式，不转换为HTML标签（第一优先级）
  * **type为"填空"的题目**：下划线必须转换为`<input size=\"X\" readonly=\"readonly\" type=\"underline\">`标签
  * **选择题填空**：无论在哪个层级，都保持原始格式`______`或`（）`
- **关键区分**：
  * **有subQuestions的question**：主题目中的content保留原始下划线`______`
  * **type为"填空"的题目**：必须转换为`<input>`标签
...
- **独立单选题**：题和题之间是相对独立的
- **普通填空题**：每道题都是独立的填空题，没有共享材料（默写题除外，默写题虽然也没用共享材料，但是需要拆分成子题的结构）
- **独立简答题**：每道题都是独立的问题
...
7. **填空题特殊要求**：
   - **格式严格匹配（重要）**：
     ...
     * **type为"填空"的题目**：必须用 `<input size=\"X\" readonly=\"readonly\" type=\"underline\">`
...
1. **🚨 填空题结构化处理检查（第一优先级）**：
   ...
   - **🚨 type为"填空"的题目检查**：所有`"type": "填空"`的题目，必须转换`______`为`<input>`标签
   - **🚨 默写题特殊检查**：包含"默写"关键词的题目必须使用subQuestions结构
   - **🚨 常见错误**：包含子题的主题目错误转换了下划线；type为"填空"的题目使用了原始下划线
2. **加点字格式转换检查（第二优先级）**：
...
7. **🚨 默写题检查**：
   ...
   - **🚨 子题填空处理**：子题type为"填空"时，下划线转换为`<input size=\"X\" readonly=\"readonly\" type=\"underline\">`格式
   - 答案与原文完全一致，不能有任何错字
